...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="container mx-auto p-6">
        <form method="POST" action="{{ route('admin.projects.update', $project->id) }}" class="bg-white rounded-lg shadow-md p-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Image</label><input type="text" name="image" value="{{ $project->image }}" class="w-full border rounded px-3 py-2"></div>
