...
@section('title', 'Project Detail Details - Admin')
@section('page-title', 'Project Detail Details')
@section('content')
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase">ID</h3>
            <p class="mt-1 text-lg text-gray-900 dark:text-white">{{ $projectDetail->id }}</p>
        </div>
        <div>
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase">Title</h3>
            <p class="mt-1 text-lg text-gray-900 dark:text-white">{{ $projectDetail->title }}</p>
        </div>
        <div>
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase">Link</h3>
            <p class="mt-1 text-lg text-gray-900 dark:text-white">{{ $projectDetail->link }}</p>
        </div>
        <div class="md:col-span-2">
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase">Description</h3>
            <p class="mt-1 text-lg text-gray-900 dark:text-white">{{ $projectDetail->description }}</p>
        </div>
    </div>
